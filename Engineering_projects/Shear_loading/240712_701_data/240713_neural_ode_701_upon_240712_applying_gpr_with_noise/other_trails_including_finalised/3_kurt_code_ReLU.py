...
optimizer = optim.RMSprop(funcKurtosis.parameters(), lr=0.005)
...
# Setting up the optimizer with all parameters
optimizer = optim.RMSprop(funcKurtosis.parameters(), lr=0.0005)
...
# Setting up the optimizer with all parameters
optimizer = optim.RMSprop(funcKurtosis.parameters(), lr=0.0001)

for itr in range(2020, 2500):
#for itr in range(1, 20):
        ## Using mini-batch data to update the neural network coefficients
        optimizer.zero_grad()
        batch_y0, batch_t, batch_y = get_batch()
...
ax.set_title("Learning evolution Kurt", fontsize=18, fontweight='bold')
...
plt.savefig('learning_3_kurt_ReLU.pdf', bbox_inches='tight')

torch.save(funcKurtosis, '3_kurtosis_ReLU.pth')
# Show the plot
# plt.show()
