...
optimizer = optim.RMSprop(funcVariance.parameters(), lr=0.005)
...
# Setting up the optimizer with all parameters
optimizer = optim.RMSprop(funcVariance.parameters(), lr=0.0001)

for itr in range(1520, 2000):
#for itr in range(1, 20):
        ## Using mini-batch data to update the neural network coefficients
        optimizer.zero_grad()
        batch_y0, batch_t, batch_y = get_batch()
...
plt.plot(tTrain,xTrain[:,1], 'g',label='Variance exp NN interpolated')
...
ax.set_title("Learning evolution of Var", fontsize=18, fontweight='bold')
...
plt.savefig('learning_1_var.pdf', bbox_inches='tight')

torch.save(funcVariance, '1_variance.pth')
